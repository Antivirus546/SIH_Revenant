...
ee.Authenticate()
ee.Initialize(project='sih-revenant')
...
# Fetch GHSL population count (~100m native resolution, matches your export scale)
population = ee.Image('JRC/GHSL/P2023A/GHS_POP/2025').rename('Population')
# Combine all 6 bands and force 32-bit float format to prevent export errors
final_image = landsat.select('ST_B10').rename('Target_Temp').addBands([ndvi, ndbi, albedo, ndwi, elevation, population]).toFloat()
